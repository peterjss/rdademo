 ...
 */
public class RFIDIchSKHandler extends SimpleChannelUpstreamHandler implements IProtocol
{

	private static final Logger logger = LoggerFactory.getLogger(RFIDIchSKHandler.class);
	...
	public RFIDIchSKHandler(IchHandlerConfig config)
	{
		this.config = config;
	}
	...
	{
		logger.info("Received the 'SK' message.");
	        ...
	        throws Exception
	{
		...
		this.sendSAResponse(bizRequest, e.getChannel(), session);
		e.getChannel().getPipeline().remove(this);
	}

	private void sendSAResponse(RFIDRequest bizRequest, Channel channel, Session session)
	{
		logger.info("Send the 'SA' response to device.");
		...
		ChannelBuffer con = (ChannelBuffer) MessageUtil.saEcodeMessage(response);
	...
	private void sendSNResponse(RFIDRequest bizRequest, Channel channel, Session session, String errorCode,
	        String errorDescription)
	{
		logger.info("Send the 'SN' response to device.");
		// Session session = Session.getCurrentSession(e.getChannel());
