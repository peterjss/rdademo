 ...
 */
public class BdbPersistenceStore<E extends Serializable> extends AbstractQueue<E> implements Serializable
{
	private static final Logger log = LoggerFactory.getLogger(BdbPersistenceStore.class);
	 ...
	 * @param classCatalog
	 */
	public BdbPersistenceStore(Database db, Class<E> valueClass, StoredClassCatalog classCatalog)
	{
		this.queueDb = db;
	 ...
	 * @param valueClass
	 */
	public BdbPersistenceStore(String dbDir, String dbName, Class<E> valueClass)
	{
		headIndex = new AtomicLong(0);
		tailIndex = new AtomicLong(0);
		this.dbDir = dbDir;
	        ...
	        throws DatabaseNotFoundException, DatabaseExistsException, DatabaseException, IllegalArgumentException
	{
		...
		try {
			...
			if (!envFile.exists()) {// 如果指定的目录不存在，则自动创建
				envFile.mkdir();
			}
			...
			if (cursor.getNext(foundKey, foundData, LockMode.DEFAULT) == OperationStatus.SUCCESS) {
				Entry<String, E> entry = new SimpleEntry<String, E>(restoreKey(foundKey), restoreData(foundData));

			}
			System.out.println("map size:" + queueMap.size());
	 ...
	 */
	private E restoreData(final DatabaseEntry entry)
	{

		final EntryBinding dataBinding = new SerialBinding(dbEnv.getClassCatalog(), dataClass);
		@SuppressWarnings("unchecked")
		final E data = (E) dataBinding.entryToObject(entry);
		return data;
	}
