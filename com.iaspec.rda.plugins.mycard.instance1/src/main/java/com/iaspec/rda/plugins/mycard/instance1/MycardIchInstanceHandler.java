...
import com.iaspec.rda.plugins.mycard.external.protocol.MycardDecoder;
import com.iaspec.rda.plugins.mycard.external.protocol.MycardEncoder;
import com.iaspec.rda.plugins.mycard.external.server.IchHandlerConfig;
import com.iaspec.rda.plugins.mycard.external.server.MycardIchHandler;
...
public class MycardIchInstanceHandler extends FrameDecoder implements IServerHandler
{
    private static final Logger logger = LoggerFactory.getLogger(MycardIchInstanceHandler.class);

    private final String handlerName = this.getClass().getSimpleName();

    private final boolean detectSsl;
    private final boolean detectGzip;
    private IMessageDispatcher messageDispatcher = MycardInstanceActivator.messageDispatcher;

    public MycardIchInstanceHandler()
    {
        this(true, true);
    }

    private MycardIchInstanceHandler(boolean detectSsl, boolean detectGzip)
    {
    ...
    protected Object decode(ChannelHandlerContext ctx, Channel channel, ChannelBuffer buffer) throws Exception
    {
        ...
        else
        {
            ...
            else if (isHttp(magic1, magic2))
            {
                ...
                switchToHttp(ctx);
            }else if (isMycard(buffer)) 
        	{
               logger.info("Detect mycard Handshake.");
               switchToMycard(ctx);
            }
            else
            {
    ...
    public void channelConnected(ChannelHandlerContext ctx, ChannelStateEvent e) throws Exception
    {
		...
		// MycardIchServer.channels.add(e.getChannel());
    ...
    private static boolean isMycard(ChannelBuffer buffer)
    {
        if (buffer.readByte() == (byte)0xFA)
        {
    ...
    private void enableSsl(ChannelHandlerContext ctx)
    {
        ...
        p.addLast("unificationA", new MycardIchInstanceHandler(false, detectGzip));
        p.remove(this);
    }
    ...
    private void enableGzip(ChannelHandlerContext ctx)
    {
        ...
        p.addLast("unificationB", new MycardIchInstanceHandler(detectSsl, false));
        p.remove(this);
    }
    ...
    private void switchToHttp(ChannelHandlerContext ctx) throws Exception
    {
        ...
        MycardIchHandler handler = new MycardIchHandler(config);
    ...
    private void switchToMycard(ChannelHandlerContext ctx) throws Exception
    {
        System.out.println("in switch.");
        ChannelPipeline p = ctx.getPipeline();
    	p.addLast("decoder", new MycardDecoder());
		p.addLast("encoder", new MycardEncoder());
